...
export default function AdminTools() {
  ...
  const complete=data.filter(
    (row) =>
      row['ID Verification'] === 'Verified' &&
      row['Portfolio Verification'] === 'Verified'
  )

  const totalPages = Math.ceil(complete.length / rowsPerPage)

  const paginatedData = complete.slice(
      ...
      <div className="w-[925px] border rounded-[15px]
       border-[#FAFAFA] px-[60px] py-[30px] overflow-hidden text-sm bg-[#FFF]">
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            <p className="text-[14px] text-[#000] font-medium">Requests Complete</p>
